<template>
  <div>
    ...
    <el-dialog title="提示" :visible.sync="previewVisible" width="50%">
      <img :src="previewPath" alt="" class="img-preview" />
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    handleChange() {
      if (this.addForm.goods_cat.length !== 3) {
        ...
        return this.$message.error('请选择第三级标签')
      }
    },
    ...
    async tabClicked() {
      if (this.activeIndex === '1') {
        const { data: res } = await this.$http.get(
          `categories/${this.careId}/attributes`,
          {
            params: { sel: 'many' }
          }
        )
        if (res.meta.status !== 200) {
          return false.$message.error('获取分类动态数据失败')
        }
        ...
        if (res.meta.status !== 200) {
          return false.$message.error('获取分类静态数据失败')
        }
        res.data.forEach(item => {
          item.attr_vals =
            item.attr_vals.length === 0 ? [] : item.attr_vals.split(' ')
